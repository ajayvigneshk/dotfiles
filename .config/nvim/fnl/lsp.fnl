...
(fn on-attach [client bufnr]
  ...
  (buf-set-keymap :n :<space>wa "<cmd>lua vim.lsp.buf.add_workspace_folder()<CR>" opts)
  (buf-set-keymap :n :<space>wr "<cmd>lua vim.lsp.buf.remove_workspace_folder()<CR>" opts)
  (buf-set-keymap :n :<space>wl "<cmd>lua print(vim.inspect(vim.lsp.buf.list_workspace_folders()))<CR>" opts)
  (buf-set-keymap :n :<space>D "<cmd>lua vim.lsp.buf.type_definition()<CR>" opts)
  (buf-set-keymap :n :<space>rn "<cmd>lua vim.lsp.buf.rename()<CR>" opts)
  (buf-set-keymap :n :gr "<cmd>lua vim.lsp.buf.references()<CR>" opts)
  (buf-set-keymap :n :<space>e "<cmd>lua vim.lsp.diagnostic.show_line_diagnostics()<CR>" opts)
  ...
  (buf-set-keymap :n :<space>q "<cmd>lua vim.lsp.diagnostic.set_loclist()<CR>" opts)

  ; Set some keybinds conditional on server capabilities
  (if client.resolved_capabilities.document_formatting
    (buf-set-keymap :n :<space>f "<cmd>lua vim.lsp.buf.formatting()<CR>" opts)
    client.resolved_capabilities.document_range_formatting
    (buf-set-keymap :n :<space>f "<cmd>lua vim.lsp.buf.range_formatting()<CR>" opts))
